...
/// already <em>derives</em> <c>publishable: false</c> on a non-native host or a latency-profile run, but
/// that alone would let an official run finish quietly with an unpublishable number, wasting the run and
/// inviting a "why isn't it publishable?" surprise. This guard asserts the precondition <em>up front</em> so
/// official mode either runs toward a genuinely publishable number or refuses immediately with an
/// actionable message. Local mode is never gated.
/// </summary>
public static class OfficialModeGuard
{
    ...
    /// Rosetta state and throws unless they are native x86-64 with the latency dial off.
    /// </summary>
    /// <param name="mode">The requested run mode.</param>
    /// <param name="latency">The latency-profile dial the run was asked to use.</param>
    /// <exception cref="OfficialModeNotSupportedException">
    /// Official mode requested on a non-x86-64 or Rosetta-emulated host, or with the latency dial engaged.
    /// </exception>
    public static void Assert(RunMode mode, LatencyProfile latency)
        => Assert(mode, latency, RuntimeInformation.ProcessArchitecture, DetectRosetta());
    ...
    /// x86-64, or that engages the latency dial - exactly the cases where
    /// <see cref="EnvironmentManifest.DerivePublishable"/> would return false.
    /// </summary>
    /// <param name="mode">The requested run mode.</param>
    /// <param name="latency">The latency-profile dial the run was asked to use.</param>
    ...
    public static void Assert(
        RunMode mode, LatencyProfile latency, Architecture processArchitecture, bool isRosettaEmulated)
    {
        ...
        if (EnvironmentManifest.DerivePublishable(mode, processArchitecture, isRosettaEmulated, latency))
        {
            return;
        }

        // The dial is reported first: it is the one ineligibility the operator asked for by hand, so naming
        // it is more useful than naming the host.
        if (latency.IsEngaged)
        {
            throw new OfficialModeNotSupportedException(
                $"Official mode cannot run with the latency profile engaged (--rtt-ms {latency.RoundTripMs:0}). " +
                "A latency-profile run is a diagnostic that prices the network hop; its numbers are never " +
                "published. Drop --rtt-ms for the official run, or keep it and use --mode local.");
        }
