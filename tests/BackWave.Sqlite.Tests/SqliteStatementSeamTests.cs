...
/// <summary>
/// Guards the seam the statement budgets are measured through: every statement the SQLite adapter
/// executes goes through one of the counted wrappers in SqliteStatementCounts.
///
/// Without this the budgets are self-deceiving. A new bare <c>command.ExecuteNonQueryAsync(...)</c> in
/// the store is a real statement - a real prepare, a real step, and on the write path a real turn
/// holding the connection's write lock - that the counter never sees, so the pinned numbers in
/// SqliteStatementBudgetTests keep passing while the adapter gets slower. That is the exact failure
/// those budgets exist to catch. The seam is the only thing standing between the two, and a seam nothing
...
/// It needs no database at all - SQLite needs no container in the first place - so it gates every build
/// of the suite.
/// </summary>
public sealed class SqliteStatementSeamTests
{
    ...
    //
    // "Round trip" is Oracle's word and a misnomer on an in-process engine, as SqliteStatementCounts
    // says at length. It is kept verbatim anyway so that one grep finds every exemption across all four
    // adapters, which is worth more than a locally accurate name only this adapter would use.
    ...
    private const string WrapperFile = "SqliteStatementCounts.cs";
    ...
    public void EveryAdapterStatementGoesThroughACountedWrapper()
    {
        ...
        Assert.True(
            offenders.Count == 0,
            $"""
             {offenders.Count} SQLite statement(s) bypass the counted wrappers, so they are invisible to
             the budgets in SqliteStatementBudgetTests:

             {string.Join(Environment.NewLine, offenders)}

             Call ExecuteNonQueryCountedAsync / ExecuteReaderCountedAsync / ExecuteScalarCountedAsync
             instead. If the statement genuinely belongs to no store operation - it runs at migration or
             at connection open, outside every operation the budgets measure - put a comment directly
             above the call that starts with "{ExemptionMarker}" and says why.
             """);
    }
    ...
    private static IEnumerable<string> AdapterSources()
    {
        ...
        Assert.True(
            adapter is not null,
            $"SQLite adapter sources not found: no directory above {AppContext.BaseDirectory} holds src/BackWave.Sqlite.");
    ...
    private static string? AdapterDirectory()
    {
        ...
        {
            var adapter = Path.Combine(dir.FullName, "src", "BackWave.Sqlite");
    ...
    // rather than trusting the file keeps a fourth, uncounted helper added to SqliteStatementCounts.cs
