...
/// <summary>
/// The Postgres adapter's store-telemetry surface: every store round-trip opens a CLIENT span carrying
...
/// </summary>
[Collection("postgres")]
public sealed class PostgresStoreTelemetryTests
{
    private static readonly DateTimeOffset T0 = new(2026, 1, 1, 0, 0, 0, TimeSpan.Zero);

    // The adapter's own constant, not a copy of it - BackWave.Postgres grants this project
    ...
    private static readonly string SourceName = PostgresDiagnostics.SourceName;
    ...
    public async Task A_faulted_round_trip_counts_a_terminal_fault_and_marks_the_span_in_error()
    {
        ...
        await using var armed = FaultArmedStore("enqueue", _ => new FaultInjectedException("enqueue"));
    ...
    public async Task A_timeout_counts_as_a_transient_fault()
    {
        ...
        await using var armed = FaultArmedStore("enqueue", _ => new TimeoutException("injected"));
    ...
    private static PostgresJobStore FaultArmedStore(string failpoint, Func<string, Exception> fault)
        => new(new PostgresStoreOptions
        {
