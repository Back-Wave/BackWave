...
/// <summary>
/// Guards the seam the round-trip budgets are measured through: every statement the SQL Server adapter
/// executes goes through one of the counted wrappers in SqlServerRoundTrips.
///
...
/// SqlServerRoundTripBudgetTests keep passing while the adapter gets slower - the exact failure those
...
/// It needs no database, so it gates every build of the suite rather than only the runs with SQL Server
/// up - which matters more here than elsewhere, since the container runs emulated on Apple Silicon.
/// </summary>
public sealed class SqlServerRoundTripSeamTests
{
    // The SqlClient methods that actually leave for the server. The adapter calling one of these
    // directly is a round trip no budget can see.
    ...
    private const string WrapperFile = "SqlServerRoundTrips.cs";
    ...
    public void EveryAdapterStatementGoesThroughACountedWrapper()
    {
        ...
        Assert.True(
            offenders.Count == 0,
            $"""
             {offenders.Count} SQL Server statement(s) bypass the counted round-trip wrappers, so they are
             invisible to the budgets in SqlServerRoundTripBudgetTests:
             ...
             connection, outside every operation the budgets measure - put a comment directly above the
    ...
    private static IEnumerable<string> AdapterSources()
    {
        ...
        Assert.True(
            adapter is not null,
            $"SQL Server adapter sources not found: no directory above {AppContext.BaseDirectory} holds src/BackWave.SqlServer.");
    ...
    private static string? AdapterDirectory()
    {
        ...
        {
            var adapter = Path.Combine(dir.FullName, "src", "BackWave.SqlServer");
    ...
    // rather than trusting the file keeps a fourth, uncounted helper added to SqlServerRoundTrips.cs from
