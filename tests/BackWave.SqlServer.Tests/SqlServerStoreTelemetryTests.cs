...
/// <summary>
/// The SQL Server adapter's store-telemetry surface: every store round-trip opens a CLIENT span
/// carrying the db.* attributes, and a faulted round-trip increments <c>backwave.store.faults</c>
/// tagged transient (the worker degrades-and-retries) vs terminal (the worker fail-stops). This is the
/// signal an operator reaches for during an incident, so the contract is pinned here rather than left
/// to the reader of the emit site.
///
/// Isolation: the store's Meter and ActivitySource are process-wide statics shared with every other
/// test in the process. Every test here therefore starts its own parent Activity and keeps only the
/// spans and measurements carrying that marker's TraceId - the store span nests under the ambient
/// Activity (it is deliberately never forced to a root), and the counter is recorded while that span is
/// current, so the TraceId cleanly separates this test's signal from everyone else's.
/// </summary>
[Collection("sqlserver")]
public sealed class SqlServerStoreTelemetryTests
{
    private static readonly DateTimeOffset T0 = new(2026, 1, 1, 0, 0, 0, TimeSpan.Zero);

    // The adapter's own constant, not a copy of it - BackWave.SqlServer grants this project
    ...
    private static readonly string SourceName = SqlServerDiagnostics.SourceName;
    ...
    public async Task A_timeout_counts_as_a_transient_fault()
    {
        ...
        // A bare TimeoutException is squarely in this adapter's transient set: the worker retries
        // rather than fail-stopping, so the counter must not report it as terminal.
        await SqlServerTestDatabase.CreateFreshStoreAsync();
    ...
    private static SqlServerJobStore FaultArmedStore(string failpoint, Func<string, Exception> fault)
        => new(new SqlServerStoreOptions
        {
