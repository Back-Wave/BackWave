...
/// <summary>
/// Guards the seam the round-trip budgets are measured through: every statement the Oracle adapter
/// executes goes through one of the counted wrappers in OracleRoundTrips.
///
...
/// OracleRoundTripBudgetTests keep passing while the adapter gets slower - the exact failure those
...
/// It needs no database, so it gates every build of the suite rather than only the runs with Oracle up.
/// </summary>
public sealed class OracleRoundTripSeamTests
{
    // The ODP.NET methods that actually leave for the server. The adapter calling one of these directly
    ...
    private const string WrapperFile = "OracleRoundTrips.cs";
    ...
    public void EveryAdapterStatementGoesThroughACountedWrapper()
    {
        ...
        Assert.True(
            offenders.Count == 0,
            $"""
             {offenders.Count} Oracle statement(s) bypass the counted round-trip wrappers, so they are
             invisible to the budgets in OracleRoundTripBudgetTests:
    ...
    // rather than trusting the file keeps a fourth, uncounted helper added to OracleRoundTrips.cs from
