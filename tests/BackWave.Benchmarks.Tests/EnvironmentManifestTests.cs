...
public sealed class EnvironmentManifestTests
{
    ...
    {
        Assert.False(EnvironmentManifest.DerivePublishable(RunMode.Local, Architecture.X64, isRosettaEmulated: false, LatencyProfile.Disabled));
    }

    [Fact]
    public void Official_mode_on_native_x64_is_publishable()
    {
        Assert.True(EnvironmentManifest.DerivePublishable(RunMode.Official, Architecture.X64, isRosettaEmulated: false, LatencyProfile.Disabled));
    }

    [Fact]
    public void Official_mode_on_apple_silicon_arm64_is_not_publishable()
    {
        Assert.False(EnvironmentManifest.DerivePublishable(RunMode.Official, Architecture.Arm64, isRosettaEmulated: false, LatencyProfile.Disabled));
    }
    ...
    public void Official_mode_under_rosetta_is_not_publishable_despite_reporting_x64()
    {
        ...
        Assert.False(EnvironmentManifest.DerivePublishable(RunMode.Official, Architecture.X64, isRosettaEmulated: true, LatencyProfile.Disabled));
    }

    [Fact]
    public void Local_mode_under_rosetta_is_not_publishable()
    {
        Assert.False(EnvironmentManifest.DerivePublishable(RunMode.Local, Architecture.X64, isRosettaEmulated: true, LatencyProfile.Disabled));
    }

    [Fact]
    public void Capture_stamps_the_live_environment_and_derives_the_flag()
    {
        var manifest = EnvironmentManifest.Capture(RunMode.Local, "PostgreSQL", "17.0", LatencyProfile.Disabled);
        ...
        Assert.Equal(
            EnvironmentManifest.DerivePublishable(
                RunMode.Local, manifest.ProcessArchitecture, manifest.IsRosettaEmulated, LatencyProfile.Disabled),
    ...
    public void Guard_allows_local_mode_on_any_architecture()
    {
        ...
        OfficialModeGuard.Assert(RunMode.Local, LatencyProfile.Disabled, Architecture.Arm64, isRosettaEmulated: false);
        OfficialModeGuard.Assert(RunMode.Local, LatencyProfile.Disabled, Architecture.X64, isRosettaEmulated: true);
    }
    ...
    public void Guard_allows_official_mode_on_native_x64()
    {
        ...
        OfficialModeGuard.Assert(RunMode.Official, LatencyProfile.Disabled, Architecture.X64, isRosettaEmulated: false);
    }
    ...
    public void Guard_refuses_official_mode_on_apple_silicon_arm64()
    {
        var exception = Assert.Throws<OfficialModeNotSupportedException>(
            () => OfficialModeGuard.Assert(RunMode.Official, LatencyProfile.Disabled, Architecture.Arm64, isRosettaEmulated: false));
    ...
    {
        var exception = Assert.Throws<OfficialModeNotSupportedException>(
            () => OfficialModeGuard.Assert(RunMode.Official, LatencyProfile.Disabled, Architecture.X64, isRosettaEmulated: true));
    ...
    public void Guard_decision_matches_the_publishable_flag_for_every_official_cell()
    {
        ...
        // official run that survives the gate is guaranteed to carry publishable: true. The latency dial is
        // swept alongside the host facts: it is an ineligibility exactly like Rosetta is.
        LatencyProfile[] dials = [LatencyProfile.Disabled, LatencyProfile.OfMilliseconds(5)];
        foreach (var arch in new[] { Architecture.X64, Architecture.Arm64 })
        {
            foreach (var rosetta in new[] { false, true })
            {
                foreach (var latency in dials)
                {
                    var publishable = EnvironmentManifest.DerivePublishable(RunMode.Official, arch, rosetta, latency);
                    Assert.Equal(
                        arch == Architecture.X64 && !rosetta && !latency.IsEngaged,
                        publishable);

                    var threw = false;
                    try
                    {
                        OfficialModeGuard.Assert(RunMode.Official, latency, arch, rosetta);
                    }
