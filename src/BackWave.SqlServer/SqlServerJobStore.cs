...
public sealed class SqlServerJobStore(SqlServerStoreOptions options) : IJobStore
{
        ...
        SqlConnection connection, SqlTransaction transaction, string queue, CancellationToken cancellationToken)
    {
        ...
        var code = (int)(await applock.ExecuteScalarCountedAsync(cancellationToken).ConfigureAwait(false))!;
        // 0 granted, 1 granted after waiting. -1 timeout and -3 deadlock victim are legal contention
        // outcomes, so they surface as the transient fault the pump already retries. Anything else
        // (-2 cancelled, -999 parameter/other error) means the lock was never taken and the claim that
        // follows would read queue config unserialized.
        if (code is -1 or -3)
        {
            throw new TimeoutException(
                $"The queue-config application lock for '{queue}' was not granted (sp_getapplock returned {code}).");
        }
        if (code < 0)
        {
            throw new InvariantViolationException(
    ...
    public async ValueTask<int> RelinquishLeasesAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        using var activity = SqlServerDiagnostics.StartStore("relinquish_leases", JobsCollection);
        try
        {
            return await RetryOnDeadlockAsync(
                () => RelinquishLeasesUntracedAsync(workerId, now, disposition, cancellationToken),
                cancellationToken).ConfigureAwait(false);
        }
        catch (Exception exception) when (exception is not OperationCanceledException)
        {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await using (var select = Cmd(
            ...
            FROM backwave.jobs WITH (UPDLOCK, ROWLOCK)
        ...
        // unchanged Attempt, atomic with the state writes. Written first, before either UPDATE takes
        // an X lock on jobs, for the reason the expiry path documents above.
