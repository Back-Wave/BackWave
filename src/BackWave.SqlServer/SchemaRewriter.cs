...
// deployment can place BackWave's objects under a custom SQL Server schema (ADR 0040). Every query in
...
internal sealed partial class SchemaRewriter
{
    ...
    // SQL Server identifiers cap at 128 characters; reject longer names rather than silently truncating.
    ...
    public SchemaRewriter(string schemaName)
    {
            ...
            || !IdentifierPattern().IsMatch(schemaName))
        {
            throw new ArgumentException(
                $"'{schemaName}' is not a valid SQL Server schema name. Use 1–{MaxSchemaLength} characters: " +
