...
// The SQL Server adapter's own OpenTelemetry surface: one ActivitySource and one Meter, both named
// "BackWave.SqlServer" and versioned with the package version, kept separate from the Core "BackWave"
...
// carrying db.system = "mssql", db.operation.name (claim/enqueue/complete/fail/expire_leases), and
// db.collection.name (the effective jobs table, honoring a custom SchemaName). We never force a root: each
...
// tracer/meter provider subscribes to "BackWave.SqlServer".
internal static class SqlServerDiagnostics
{
    internal const string SourceName = "BackWave.SqlServer";

    // The db.system value for this provider, per the OTel database conventions.
    private const string DbSystemValue = "mssql";
    ...
    {
        var informational = typeof(SqlServerDiagnostics).Assembly
