...
// The Postgres adapter's own OpenTelemetry surface: one ActivitySource and one Meter, both named
// "BackWave.Postgres" and versioned with the package version, kept separate from the Core "BackWave"
...
// carrying db.system = "postgresql", db.operation.name (claim/enqueue/complete/fail/expire_leases), and
...
// tracer/meter provider subscribes to "BackWave.Postgres".
internal static class PostgresDiagnostics
{
    internal const string SourceName = "BackWave.Postgres";
    ...
    {
        var informational = typeof(PostgresDiagnostics).Assembly
