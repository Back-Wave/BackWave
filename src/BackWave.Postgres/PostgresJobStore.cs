...
public sealed class PostgresJobStore : IJobStore, IWakeUpHintSource, IAsyncDisposable
{
    ...
    // Runs inside the caller's transaction, so the whole batch is atomic with the lease/outcome
    // write, exactly as the per-row calls were. The global Position rides the job_transitions
    // SEQUENCE default, one nextval per row. One set-based DELETE prunes the batch to
    // MaxTransitionsPerJob (§7). Honors the history policy: Off writes nothing; Transitions writes
    // the rows but never the detail; the full rung keeps the clamped detail. `now` is always the
    // caller's clock.
        ...
        WorkflowListQuery query, CancellationToken cancellationToken = default)
    {
        ...
        // states are read, so the work is bounded by the page, not the store.
        await using var connection = await _dataSource.OpenConnectionAsync(cancellationToken).ConfigureAwait(false);
        ...
        await using (var workflows = new NpgsqlCommand { Connection = connection })
        {
            ...
            if (query.After is { } after)
            {
                where = newestFirst
                    ...
                    : "WHERE created_at > @at OR (created_at = @at AND workflow_id > @id) ";
                workflows.Parameters.AddWithValue("at", after.CreatedAt.ToUniversalTime());
            ...
            workflows.CommandText = _schema.Rewrite(
                $"SELECT workflow_id, name, created_at, restarted_from FROM backwave.workflows {where}{order} LIMIT @take");
            workflows.Parameters.AddWithValue("take", query.ClampedMaxResults(_options.Bounds));
            await using var reader = await workflows.ExecuteReaderCountedAsync(cancellationToken).ConfigureAwait(false);
            while (await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
            {
                page.Add((
        ...
        {
            await using var members = Cmd(
                "SELECT workflow_id, state FROM backwave.jobs WHERE workflow_id = ANY(@ids)", connection);
            members.Parameters.AddWithValue("ids", page.Select(w => w.WorkflowId).ToArray());
            await using var reader = await members.ExecuteReaderCountedAsync(cancellationToken).ConfigureAwait(false);
            while (await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
            {
                var wf = reader.GetGuid(0);
