...
// The Oracle adapter's own OpenTelemetry surface: one ActivitySource and one Meter, both named
// "BackWave.Oracle" and versioned with the package version, kept separate from the Core "BackWave"
...
// carrying db.system = "oracle", db.operation.name (claim/enqueue/complete/fail/expire_leases), and
// db.collection.name (the effective jobs table, honoring a custom SchemaName). We never force a root:
// each span inherits whatever Core span is ambient at its call site - the PRODUCER "send" span for
// enqueue, the CLIENT "receive" span for claim - while the background lease sweep, running under no job
// span, is simply a root.
//
...
// tracer/meter provider subscribes to "BackWave.Oracle".
internal static class OracleDiagnostics
{
    internal const string SourceName = "BackWave.Oracle";

    // The db.system value for this provider, per the OTel database conventions.
    private const string DbSystemValue = "oracle";
    ...
    {
        var informational = typeof(OracleDiagnostics).Assembly
