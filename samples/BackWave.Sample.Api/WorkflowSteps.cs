...
// Workflow steps (ADR 0023) for the sample API. A step is an ordinary [Job] payload record wearing
// IWorkflowStep, so the strongly-typed workflow builder can chain it by its .NET type. Each has an
// ordinary IJobHandler<T>; the handlers just log and hold briefly so the graph view at
// /backwave/workflows/{id} shows members moving through their states. Distinct wire names per stage keep
// each node legible in the graph.
...
/// <summary>
/// Fan-out branch A: charge the card. With <see cref="ChargePayment.Fail"/> it throws every attempt, so
/// it Dead-Letters - its on-success dependents Cancel, and the Workflow projects Failed (failure
/// dominates) in the graph view.
/// </summary>
...
/// <summary>
/// A structured "document" job whose flat fields render as readable JSON in the dashboard's payload card.
/// On the 'low' queue (Weighted group, MaxAttempts=3) so the <see cref="OrderNotification.Fail"/> path
/// Dead-Letters quickly: when true the handler throws every attempt, exercising Failure Detail capture and
/// a multi-entry timeline. Used both as the terminal step of the order-fulfillment workflow and standalone.
/// </summary>
...
// ── Job Output diamond: ingest ─> {enrich, score} ─> publish (River's LoadDeps, ADR 0026) ─────
// A handler emits one opaque blob via SetOutput on success, and a descendant PULLS the output of its
// transitive Dependency ancestors via GetDependencyOutputAsync - never injected (pull, never push).
...
public sealed class IngestHandler(ILogger<IngestHandler> logger) : IJobHandler<Ingest>
{
    public async Task HandleAsync(Ingest job, JobContext context, CancellationToken cancellationToken)
    {
        ...
        await Task.Delay(400, cancellationToken);
        context.SetOutput(new DatasetSummary(job.DatasetRef, RowCount: 1_000), SampleOutputJsonContext.Default.DatasetSummary);
    }
}

/// <summary>Fan-out branch A: pulls its parent <c>ingest</c>'s output, uses it, then emits its own.</summary>
...
public sealed class EnrichHandler(ILogger<EnrichHandler> logger) : IJobHandler<Enrich>
{
    public async Task HandleAsync(Enrich job, JobContext context, CancellationToken cancellationToken)
    {
        var ingest = await context.GetDependencyOutputAsync(
            "ingest", SampleOutputJsonContext.Default.DatasetSummary, cancellationToken);
        ...
        await Task.Delay(400, cancellationToken);
        context.SetOutput(new EnrichedResult(rows, $"enriched {job.DatasetRef}"), SampleOutputJsonContext.Default.EnrichedResult);
    }
}
...
public sealed class ScoreHandler(ILogger<ScoreHandler> logger) : IJobHandler<Score>
{
    public async Task HandleAsync(Score job, JobContext context, CancellationToken cancellationToken)
    {
        ...
        await Task.Delay(400, cancellationToken);
        context.SetOutput(new ScoreResult(value), SampleOutputJsonContext.Default.ScoreResult);
    }
}

/// <summary>
/// Fan-in: pulls the whole upstream - its direct parents <c>enrich</c> + <c>score</c> AND, TRANSITIVELY,
/// their shared grandparent <c>ingest</c> (the headline LoadDeps move). It branches on the ancestor's
/// terminal state, and shows that a handle OUTSIDE the ancestor set resolves to a clean empty result
/// (no throw), never a racy read of a parallel branch.
/// </summary>
...
public sealed class PublishHandler(ILogger<PublishHandler> logger) : IJobHandler<Publish>
{
    ...
    {
        var ingest = await context.GetDependencyOutputAsync(
            "ingest", SampleOutputJsonContext.Default.DatasetSummary, cancellationToken);   // transitive ancestor (grandparent)
        var enriched = await context.GetDependencyOutputAsync(
            "enrich", SampleOutputJsonContext.Default.EnrichedResult, cancellationToken);   // direct parent
        var score = await context.GetDependencyOutputAsync(
            "score", SampleOutputJsonContext.Default.ScoreResult, cancellationToken);       // direct parent

        // Scope guarantee + absence-is-normal: 'validate-order' belongs to a different workflow, so it is
        // not in publish's ancestor set - unresolvable, returned as a clean empty result, never a throw.
        var outside = await context.GetDependencyOutputAsync(
            "validate-order", SampleOutputJsonContext.Default.DatasetSummary, cancellationToken);

        logger.LogInformation(
            "publish: {DatasetRef} - ingest(transitive)={IngestRows} rows [{IngestState}], enrich={EnrichRows} rows, score={Score}, non-ancestor resolved={OutsidePresent} (job {JobId})",
            job.DatasetRef,
        ...
        if (ingest.AncestorState == JobState.Succeeded)
        {
            logger.LogInformation("publish: ingest Succeeded → publishing {DatasetRef}", job.DatasetRef);
        }
