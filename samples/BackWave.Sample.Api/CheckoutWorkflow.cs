...
// The all-features Workflows v2 sample (the /workflows/checkout endpoint). One "checkout" graph that
// exercises every new v2 shape at once, so the dashboard graph view at /backwave/workflows/{id} can be
// confirmed to render each: a PARALLEL fan-out, a fan-in, a saga COMPENSATION side-branch, a seed-aware
// CONDITIONAL gate (which cancels the not-taken arm), a converge past the conditional, a spliced CHILD
// workflow, and typed Job Output feeding the gate. Each step is an ordinary [Job] payload wearing
// IWorkflowStep with a plain IJobHandler<T>; the handlers log and hold their lease briefly so the members
// are watchable moving through their states. Distinct wire names per stage keep every node legible.
//
...
// ran Succeeded - read per-step state, not the rollup (see the endpoint note in Program.cs).
